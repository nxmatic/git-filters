...
META[yqFormat]=${3:-$( git::sops:input:yq:format ${META[fileExt]} )}

if [[ -n "${META[yqFormat]}" && "${META[yqFormat]}" != "unsupported" ]]; then
  git::sops::encrypt() {
  ...
  git::sops::decrypt() {
    ...
    yq -i yaml -o ${META[yqFormat]} eval . /dev/stdin
  }
else
  git::sops::encrypt() {
...
case "${OP}" in
  ...
  "clean")
    ...
    if [[ -z "${ENCRYPTED_HEAD_CONTENTS}" || "${DECRYPTED_HEAD_CONTENTS}" != "${INPUT}" ]]; then
      git::sops encrypt <<<"${INPUT}"
      ENCRYPTED_INPUT="$( git::sops encrypt <<<"${INPUT}" 2>/dev/null )"
      git::sops show "${ENCRYPTED_INPUT}"
    else
      git::sops show "${ENCRYPTED_HEAD_CONTENTS}"
    fi;;
  *)
    exit 1;;
esac
